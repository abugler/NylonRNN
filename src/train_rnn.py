...
def train_LSTM(model, features, targets, batch_size=20, regular_param=1e-8, learning_rate=.003,
               lr_decay=.999, loss=nn.BCELoss(), regularization=nn.MSELoss(), model_name=None,
    ...
    :param lr: Initial Learning Rate for optimizer
    ...
    :return lr_decay: Multiplies learning rate by this amount each epoch.
    """
...
for i in range(len(list_songs)):
    ...
    next_beat = find_beat_matrix(next_array, 0)
...
model = NylonRNN(60, 50, n_steps=10000, n_layers=15)
...
model, training_loss, model_name = train_LSTM(model, training_features, training_targets,
                                  batch_size=1, learning_rate=1e-3, regular_param=0)
